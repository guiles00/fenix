<?
	...
	/*Trae los perfiles que tiene el usuario*/
		function getUserPerfil()
		{
			...
			$strSQL = "SELECT *
FROM usuario_perfil up, perfil p
WHERE up.perfil_id = p.perfil_id and up.usuario_id=" . floatval($this->userID) ;
			$data = $this->query->executeQuery($strSQL);
			...
			while($row = mysql_fetch_array($data))
			{
				...
				$resp[$row['nombre']] = array($row['perfil_id'],$row['nombre']);
			}
			
			return $resp;
		}
		
	function getUserPerfilTemp()
		{
