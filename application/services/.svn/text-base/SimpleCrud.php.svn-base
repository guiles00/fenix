...
class Services_SimpleCrud
{
	...
	public function findByName(Model $model, $criterio){
	...
	public function getById(Model $model, Array $id){
	...
	public function save(Model $model, $values){
		...
		$model_base = ($values['id']==null)?$model :$model->getTable()->find($values['id']);
	...
	public function delete(Model $model, Int $id){
		...
		$model_base = $model->getTable()->find($id);
		$model_base->delete();
