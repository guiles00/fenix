 ...
 *
 * @category   MyZend
 * @package    MyZend_Doctrine
 * @subpackage MyZend_Doctrine_Auth_Adapter
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class MyZend_Doctrine_Auth_Adapter_Adapter implements Zend_Auth_Adapter_Interface
{
	/**
	 * Database Connection
	 *
	 ...
	 * @return MyZend_Doctrine_Auth_Adapter Provides a fluent interface
	 */
...
//exit;
	...
	protected function _authenticateSetup()
	{
		$exception = null;
//echo "<pre>";
//print_r($this->_tableName);
//exit;
		if ($this->getConnection() === null) {
			$exception = 'A database connection was not set, nor could one be created.';
		} elseif ($this->_tableName == '') {
			$exception = 'A table must be supplied for the MyZend_Doctrine_Auth_Adapter authentication adapter.';
		} elseif ($this->_identityColumn == '') {
			$exception = 'An identity column must be supplied for the MyZend_Doctrine_Auth_Adapter authentication adapter.';
		} elseif ($this->_credentialColumn == '') {
			$exception = 'A credential column must be supplied for the MyZend_Doctrine_Auth_Adapter authentication adapter.';
		} elseif ($this->_identity == '') {
			$exception = 'A value for the identity was not provided prior to authentication with MyZend_Doctrine_Auth_Adapter.';
		} elseif ($this->_credential === null) {
			$exception = 'A credential value was not provided prior to authentication with MyZend_Doctrine_Auth_Adapter.';
		}
//echo "<pre>";
//print_r($exception);
//exit;
...
//exit;
   ...
   /* $dbSelect = Doctrine_Query::create()
		->select('COUNT('.$this->_credentialColumn.') ')
		->from('Model_Usuario u')
		->addWhere('u.username = ?', $this->_identity)
		->andWhere('u.password = ?', $this->_credential);
    */
    //print_r($dbSelect->getSqlQuery());
    //exit;
    
		
    
		$dbSelect = Doctrine_Query::create($this->getConnection())
		->select('COUNT('.$this->_credentialColumn.') as zend_auth_credential_match')
		->from($this->_tableName)
		->from('Model_Usuario u')
		->addWhere($this->_identityColumn .' = ?', $this->_identity)
		->andWhere($this->_credentialColumn . ' = ' . str_replace('?',$this->getConnection()->quote($this->_credential), $this->_credentialTreatment));
...
//exit;
			 ...
			 */
			require_once 'Zend/Auth/Adapter/Exception.php';
			throw new Zend_Auth_Adapter_Exception('Guillito The supplied parameters to Zend_Auth_Adapter_Doctrine_Record failed to '
