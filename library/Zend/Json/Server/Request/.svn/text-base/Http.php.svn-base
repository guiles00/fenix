...
class Zend_Json_Server_Request_Http extends Zend_Json_Server_Request
{
     ...
     * @var string
     */
    protected static $_rawJson=null; //agrege static
	public static $version_modificada_para_persistir=1;
    ...
    public function __construct()
    {
        ...
        if (self::$_rawJson==null)
        {
        	self::$_rawJson = $json;
        }//$this->_rawJson = $json;
        if (!empty($json)) {
        	$s=new Zend_Json_Server_Request();
            /*$this*/$s->loadJson($json);
        }
    }
