//gseguros.poliza.poliza.endoso-poliza-judiciales.js
$(document).ready(
		function() {
			function confirmar() {
...
//calcula importe - plus - premio asegurado
			...
			.change(
					function() {
						...
						s_importe = $('#importe_judiciales_endoso');
			...
			$('#valor_cuota_judiciales_endoso')
					.change(
							function() {

								var s_cuotas = $('#cuotas_judiciales_endoso'),
								s_valor_cuota = $('#valor_cuota_judiciales_endoso');
								var importe=0;
								var cuotas=0;
								var valor_cuota = 0;
								...
								if ((cuotas != 0 && valor_cuota != 0)) {
									...
									$('#importe_judiciales_endoso').val(importe);
								}

							});

			$('#cuotas_judiciales_endoso')
					.change(
							function() {
			...
			// Guardar Solicitud
			$('#save_endoso_poliza_judiciales').click(function() {
								...
								var data = $('#endoso_poliza_judiciales').serializeArray();
								// console.debug(data);

								$.ajax({
											url : "./poliza/poliza/endoso-poliza-judiciales",
											data : data,
			...
			$('#enviar_compania_endoso_poliza_judiciales').click(function() {
				...
				$.ajax({
					url : "./poliza/poliza/enviar-solicitud-endoso-compania-judiciales",
					data : {
						poliza_id : poliza_id
					},
					success : function(result) {
