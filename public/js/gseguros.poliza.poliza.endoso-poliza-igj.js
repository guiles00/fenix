//gseguros.poliza.poliza.endoso-poliza-igj.js
$(document).ready(
		function() {
			function confirmar() {
...
//calcula importe - plus - premio asegurado
			...
			$('#valor_cuota_igj_endoso')
					.change(
							function() {

								var s_cuotas = $('#cuotas_igj_endoso'),
								s_valor_cuota = $('#valor_cuota_igj_endoso');
								var importe=0;
								var cuotas=0;
								var valor_cuota = 0;
								...
								if ((cuotas != 0 && valor_cuota != 0)) {
									...
									$('#importe_igj_endoso').val(importe);
								}
			...
			// Guardar Solicitud
			$('#save_endoso_poliza_igj').click(function() {
								...
								var data = $('#endoso_poliza_igj').serializeArray();
								// console.debug(data);

								$.ajax({
											url : "./poliza/poliza/endoso-poliza-igj",
											data : data,
			...
			$('#enviar_compania_endoso_poliza_igj').click(function() {
				...
				$.ajax({
					url : "./poliza/poliza/enviar-solicitud-endoso-compania-igj",
					data : {
						poliza_id : poliza_id
					},
					success : function(result) {
