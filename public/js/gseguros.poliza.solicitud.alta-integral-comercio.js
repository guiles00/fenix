$(document).ready(
		function() {
			...
			$('#valor_cuota_integral_comercio')
					.change(
							function() {

								var s_cuotas = $('#cuotas_integral_comercio'),
								s_valor_cuota = $('#valor_cuota_integral_comercio');
								var importe=0;
								var cuotas=0;
								var valor_cuota = 0;
								...
								if ((cuotas != 0 && valor_cuota != 0)) {
									...
									$('#importe_integral_comercio').val(importe);
								}

							});

			$('#cuotas_integral_comercio')
					.change(
							function() {
		 ...
		 $('#fecha_pedido_agente').val(date_today);
			...
			// Guardar Solicitud
			$('#save_solicitud_integral_comercio').click(function() {
				
				if(!$("#solicitud_poliza_integral_comercio").valid())return false;	 
								...
								var data = $('#solicitud_poliza_integral_comercio').serializeArray();
								// console.debug(data);

								$.ajax({
											url : "./poliza/solicitud/alta-integral-comercio",
											data : data,
			...
			$('#confirmar_solicitud_integral_comercio').click(function() {
				...
				var importe_integral_comercio = $('#importe_integral_comercio').val();
				if(importe_integral_comercio <= 0 ){
					alert('No se puede crear la poliza - El importe debe ser mayor a cero!');
					return false;
				}
				//console.debug(importe_integral_comercio);
			...
			$('#enviar_compania_solicitud_integral_comercio').click(function() {
				...
				$.ajax({
					url : "./poliza/solicitud/enviar-solicitud-compania-integral_comercio",
					data : {
						solicitud_id : solicitud_id
					},
					success : function(result) {
...
//validar formulario de alta solicitud
			$("#solicitud_poliza_integral_comercio").validate({
			    rules: {
			        asegurado: {
			            required: true
			        }
