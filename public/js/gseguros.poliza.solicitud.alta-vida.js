$(document).ready(
		function() {
			...
			//$('#observaciones_seguro_vida').toggle();
			//$('#valores_seguro_vida').toggle();

			$('#datos_seguro_vida_show').click(function() {
				$('#datos_seguro').toggle();
			});
			$('#detalle_riesgo_vida_show').click(function() {
				$('#detalle_riesgo').toggle();
			});
			$('#datos_solicitud_vida_show').click(function() {
				$('#datos_solicitud').toggle();
			});
			$('#valores_seguro_vida_show').click(function() {
				$('#valores_seguro_vida').toggle();
			});
			...
			$('#observaciones_seguro_vida_show').click(function() {
				$('#observaciones_seguro_vida').toggle();
			});
			$('#detalle_riesgo_vida_show').click(function() {
				$('#detalle_riesgo_vida').toggle();
			});
			...
			$('#valor_cuota_vida')
					.change(
							function() {
								...
								var s_cuotas = $('#cuotas_vida'),
								s_valor_cuota = $('#valor_cuota_vida');
								var importe=0;
								var cuotas=0;
								var valor_cuota = 0;
		 ...
		 $('#fecha_pedido_agente').val(date_today);
			...
			// Guardar Solicitud
			$('#save_solicitud_vida').click(function() {
				
				if(!$("#solicitud_poliza_vida").valid())return false;	 
								...
								var data = $('#solicitud_poliza_vida').serializeArray();
								// console.debug(data);

								$.ajax({
											url : "./poliza/solicitud/alta-vida",
											data : data,
			...
			// Aprobar Solicitud
			$('#aprobar_solicitud_vida').click(function() {
			...
			// Confirmar Solicitud
			$('#confirmar_solicitud_vida').click(function() {
				...
				var importe_vida = $('#importe_vida').val();
				if(importe_vida <= 0 ){
			...
			// Anular Solicitud
			$('#anular_solicitud_vida').click(function() {
...
//validar formulario de alta solicitud
			$("#solicitud_poliza_vida").validate({
			    rules: {
			        asegurado: {
			            required: true
			        }
			...
			$('#enviar_compania_solicitud_vida').click(function() {
				...
				$.ajax({
					url : "./poliza/solicitud/enviar-solicitud-compania-vida",
					data : {
						solicitud_id : solicitud_id
					},
					success : function(result) {
