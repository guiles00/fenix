//gseguros.poliza.poliza.endoso-poliza-aduaneros.js
$(document).ready(
		function() {
			function confirmar() {
...
//calcula importe - plus - premio asegurado
			...
			.change(
					function() {
						...
						s_importe = $('#importe_aduaneros_endoso');
			...
			$('#valor_cuota_aduaneros_endoso')
					.change(
							function() {

								var s_cuotas = $('#cuotas_aduaneros_endoso'),
								s_valor_cuota = $('#valor_cuota_aduaneros_endoso');
								var importe=0;
								var cuotas=0;
								var valor_cuota = 0;
								...
								if ((cuotas != 0 && valor_cuota != 0)) {
									...
									$('#importe_aduaneros_endoso').val(importe);
								}

							});

			$('#cuotas_aduaneros_endoso')
					.change(
							function() {
			...
			// Guardar Solicitud
			$('#save_endoso_poliza_aduaneros').click(function() {
								...
								var data = $('#endoso_poliza_aduaneros').serializeArray();
								// console.debug(data);

								$.ajax({
											url : "./poliza/poliza/endoso-poliza-aduaneros",
											data : data,
			...
			$('#enviar_compania_endoso_poliza_aduaneros').click(function() {
				...
				$.ajax({
					url : "./poliza/poliza/enviar-solicitud-endoso-compania-aduaneros",
					data : {
						poliza_id : poliza_id
					},
					success : function(result) {
