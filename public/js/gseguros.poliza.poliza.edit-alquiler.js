$(document).ready(
		function() {
			...
			$('#valor_cuota_alquiler')
					.change(
							function() {

								var s_cuotas = $('#cuotas_alquiler'),
								s_valor_cuota = $('#valor_cuota_alquiler');
								var importe=0;
								var cuotas=0;
								var valor_cuota = 0;
...
//calcula importe - plus - premio asegurado
			...
			// Guardar poliza
			$('#save_poliza_alquiler').click(function() {
				
				//if(!$("#poliza_poliza_alquiler").valid())return false;	 
								...
								var data = $('#poliza_poliza_alquiler').serializeArray();
							 //console.debug(data);

								$.ajax({
											url : "./poliza/poliza/edit-poliza-alquiler",
											data : data,
			...
			// Anular poliza
			$('#anular_poliza_alquiler').click(function() {
...
//validar formulario de alta poliza
			$("#poliza_poliza_alquiler").validate({
			    rules: {
			        asegurado: {
			            required: true
			        }
			...
			$('#enviar_compania_poliza_alquiler').click(function() {
				...
				$.ajax({
					url : "./poliza/poliza/enviar-poliza-compania-alquiler",
					data : {
						poliza_id : poliza_id
					},
					success : function(result) {
